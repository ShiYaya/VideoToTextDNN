        ...
        for row in rows[:num_test]:
            ...
            if len(row)>5:
                ...
                if vids_names.has_key(vid_name):
                    ...
                    feat_path = '/p/work2/projects/ryat/datasets/vid-desc/lsmdc16/videos/'+vid_name+'.avi'
...
if __name__=='__main__':
    ...
    creation_args.add_argument('-d','--data_dir',dest ='data_dir', help='Example: /path/to/dataset/annotations', required=True)
    creation_args.add_argument('-p','--pkl_dir',dest ='pkl_dir', help='Example: /path/to/dataset/pkls', required=True)
    creation_args.add_argument('-dbname','--dbname',dest ='dbname', help='Dataset type.', required=True, choices=['mvad', 'mpii', 'lsmdc16'])
    ...
    ut_args.add_argument('-t', '--unit_test', dest='unit_test', type=int, default=0, help='Perform small test. Takes number of samples in unit test dataset.')
    ut_args.add_argument('-l', '--local_dir', dest='local_dir', help="Where to copy unit_test features.", default=None)
    ut_args.add_argument('-sp', '--split', dest='split', nargs='+', help='Space delimited [train val test] Data split to use in unit test dataset', default=[0.50,0.25,0.25], type=float)
    ut_args.add_argument('-feat','--feats_dir',dest ='feats_dir', help='Example: /path/to/dataset/features_googlenet', required=False)
    ...
    if not args.feats_dir:
        if args.local_dir:
            logger.critical("You must provide an argument for --feats_dir to create a local copy of features (--local_dir)")
            sys.exit(1)
